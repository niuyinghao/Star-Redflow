...
<ui:composition template="/layouts/default.xhtml">
    ...
    <ui:define name="body">
        ...
        <#elseif c2j.isComponent(field)>
	        <#foreach column in field.getColumnIterator()>
		        <#if column.value.typeName == "java.util.Date" || column.value.typeName == "date">
			        <#lt/>    <h:outputText value="${'#'}{${pojoNameLower}.${field.name}.${column}" escape="true"/>
		        <#elseif field.value.typeName == "boolean">
			        <#lt/>    <h:selectBooleanCheckbox value="${'#'}{${pojoNameLower}Form.${pojoNameLower}.${field.name}.${column}}" id="${field.name}.${column}" disabled="disabled"/>
		        <#else>
			        <#lt/>    <h:outputText value="${'#'}{${pojoNameLower}.${field.name}.${column}}" escape="true"/>
		        </#if>
	        </#foreach>
